import { Edit2, Trash2, TrendingUp, Tag, Calendar, CreditCard } from 'lucide-react';
...
const IncomeList = ({ incomes, onEdit, onDelete, loading }: IncomeListProps) => {
  if (loading) {
    return (
  ...
  if (incomes.length === 0) {
    return (
      <div className="bg-white rounded-3xl p-12 text-center border border-slate-100 shadow-lg">
        <div className="bg-emerald-50 inline-flex p-4 rounded-full mb-4">
          <TrendingUp className="text-emerald-300" size={40} />
        </div>
        <h3 className="text-xl font-semibold text-slate-700 mb-2">Nenhuma receita registrada</h3>
        <p className="text-slate-500">Comece a adicionar suas receitas pessoais</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-3xl p-6 md:p-8 shadow-xl border border-slate-100">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h3 className="text-xl font-bold text-slate-800">Receitas Recentes</h3>
      </div>
        ...
        {incomes.map((income) => (
          ...
          >
            <div className="flex items-start gap-4 mb-3 md:mb-0">
              ...
              <div>
                <h4 className="font-semibold text-slate-800">{income.description || income.category}</h4>
            ...
            <div className="flex items-center gap-3 w-full md:w-auto">
              <span className="text-2xl font-extrabold text-emerald-600 flex-1 md:flex-none">
